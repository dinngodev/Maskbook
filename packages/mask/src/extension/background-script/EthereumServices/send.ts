...
function getError(error: unknown, response?: JsonRpcResponse | null, fallback?: string): Error {
    ...
    //#region handle the response error
    const respnoseError = response?.error as unknown
    if (respnoseError instanceof Error) return getError(respnoseError, null, fallback)
    //#endregion
